...
func (cq *CandidateQuery) QueryElection() *ElectionQuery {
	query := (&ElectionClient{config: cq.config}).Query()
...
func (cq *CandidateQuery) QueryVotes() *VoteQuery {
...
func (cq *CandidateQuery) IDs(ctx context.Context) (ids []int, err error) {
...
func (cq *CandidateQuery) Clone() *CandidateQuery {
	...
	return &CandidateQuery{
		...
		withElection: cq.withElection.Clone(),
...
func (cq *CandidateQuery) WithElection(opts ...func(*ElectionQuery)) *CandidateQuery {
	query := (&ElectionClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	cq.withElection = query
	return cq
}
...
func (cq *CandidateQuery) WithVotes(opts ...func(*VoteQuery)) *CandidateQuery {
	query := (&VoteClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CandidateQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Candidate, error) {
	...
	if query := cq.withElection; query != nil {
		if err := cq.loadElection(ctx, query, nodes, nil,
			func(n *Candidate, e *Election) { n.Edges.Election = e }); err != nil {
			return nil, err
		}
	}
	if query := cq.withVotes; query != nil {
		if err := cq.loadVotes(ctx, query, nodes,
			func(n *Candidate) { n.Edges.Votes = []*Vote{} },
			func(n *Candidate, e *Vote) { n.Edges.Votes = append(n.Edges.Votes, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CandidateQuery) loadElection(ctx context.Context, query *ElectionQuery, nodes []*Candidate, init func(*Candidate), assign func(*Candidate, *Election)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Candidate)
	for i := range nodes {
		if nodes[i].election_candidates == nil {
			continue
		}
		fk := *nodes[i].election_candidates
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "election_candidates" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (cq *CandidateQuery) loadVotes(ctx context.Context, query *VoteQuery, nodes []*Candidate, init func(*Candidate), assign func(*Candidate, *Vote)) error {
	...
	query.Where(predicate.Vote(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(candidate.VotesColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.candidate_votes
		if fk == nil {
			return fmt.Errorf(`foreign-key "candidate_votes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "candidate_votes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
