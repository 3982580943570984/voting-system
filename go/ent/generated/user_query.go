...
func (uq *UserQuery) QueryElections() *ElectionQuery {
	query := (&ElectionClient{config: uq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (uq *UserQuery) WithElections(opts ...func(*ElectionQuery)) *UserQuery {
	query := (&ElectionClient{config: uq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (uq *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := uq.withElections; query != nil {
...
func (uq *UserQuery) loadElections(ctx context.Context, query *ElectionQuery, nodes []*User, init func(*User), assign func(*User, *Election)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.withFKs = true
	query.Where(predicate.Election(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.ElectionsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.user_elections
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_elections" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
