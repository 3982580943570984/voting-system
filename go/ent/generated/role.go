...
type Role struct {
	...
	// IsVoter holds the value of the "is_voter" field.
...
func (*Role) scanValues(columns []string) ([]any, error) {
	...
	for i := range columns {
		switch columns[i] {
		case role.FieldIsVoter, role.FieldIsOrganizer:
	...
	for i := range columns {
		...
		case role.FieldIsVoter:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_voter", values[i])
			} else if value.Valid {
				r.IsVoter = value.Bool
			}
		case role.FieldIsOrganizer:
			if value, ok := values[i].(*sql.NullBool); !ok {
				return fmt.Errorf("unexpected type %T for field is_organizer", values[i])
			} else if value.Valid {
				r.IsOrganizer = value.Bool
			}
...
func (r *Role) String() string {
	...
	builder.WriteString("is_voter=")
	builder.WriteString(fmt.Sprintf("%v", r.IsVoter))
	builder.WriteString(", ")
	builder.WriteString("is_organizer=")
	builder.WriteString(fmt.Sprintf("%v", r.IsOrganizer))
	builder.WriteByte(')')
