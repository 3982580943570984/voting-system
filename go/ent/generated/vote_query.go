...
func (vq *VoteQuery) QueryCandidate() *CandidateQuery {
	query := (&CandidateClient{config: vq.config}).Query()
...
func (vq *VoteQuery) Clone() *VoteQuery {
	...
	return &VoteQuery{
		...
		withCandidate: vq.withCandidate.Clone(),
...
func (vq *VoteQuery) WithCandidate(opts ...func(*CandidateQuery)) *VoteQuery {
	query := (&CandidateClient{config: vq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	vq.withCandidate = query
	return vq
}
...
func (vq *VoteQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Vote, error) {
	var (
		...
		_spec       = vq.querySpec()
		loadedTypes = [2]bool{
			vq.withCandidate != nil,
			vq.withUser != nil,
		}
	)
	if vq.withCandidate != nil || vq.withUser != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := vq.withCandidate; query != nil {
		if err := vq.loadCandidate(ctx, query, nodes, nil,
			func(n *Vote, e *Candidate) { n.Edges.Candidate = e }); err != nil {
			return nil, err
		}
	}
...
func (vq *VoteQuery) loadCandidate(ctx context.Context, query *CandidateQuery, nodes []*Vote, init func(*Vote), assign func(*Vote, *Candidate)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Vote)
	for i := range nodes {
		if nodes[i].candidate_votes == nil {
			continue
		}
		fk := *nodes[i].candidate_votes
	...
	for _, n := range neighbors {
		...
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "candidate_votes" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
