...
func createElectionTag(w http.ResponseWriter, r *http.Request) {
	...
	tc := repositories.TagCreate{ElectionId: electionId}
	if err := json.NewDecoder(r.Body).Decode(&tc); err != nil {
		http.Error(w, "Неверное тело запроса: "+err.Error(), http.StatusBadRequest)
		return
	}

	tag, err := repositories.NewTags().Create(r.Context(), &tc)
	if err != nil {
...
func getElectionTags(w http.ResponseWriter, r *http.Request) {
	...
	tags, err := repositories.NewTags().GetByElectionId(r.Context(), electionId)
	if err != nil {
...
func updateElectionTags(w http.ResponseWriter, r *http.Request) {
	...
	tu := repositories.TagsUpdate{ElectionId: electionId}
	...
	err = repositories.NewTags().UpdateByElectionId(r.Context(), &tu)
	if err != nil {
