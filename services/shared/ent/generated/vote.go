...
func (e VoteEdges) CandidateOrErr() (*Candidate, error) {
	if e.Candidate != nil {
	...
	for i := range columns {
		...
		case vote.ForeignKeys[0]:
			if value, ok := values[i].(*sql.NullInt64); !ok {
				return fmt.Errorf("unexpected type %T for edge-field candidate_votes", value)
			} else if value.Valid {
				v.candidate_votes = new(int)
				*v.candidate_votes = int(value.Int64)
			}
...
// QueryCandidate queries the "candidate" edge of the Vote entity.
func (v *Vote) QueryCandidate() *CandidateQuery {
	return NewVoteClient(v.config).QueryCandidate(v)
}
...
func (v *Vote) Unwrap() *Vote {
	...
	if !ok {
		panic("generated: Vote is not a transactional entity")
	}
