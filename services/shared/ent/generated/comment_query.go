...
func (cq *CommentQuery) QueryElection() *ElectionQuery {
...
func (cq *CommentQuery) WithElection(opts ...func(*ElectionQuery)) *CommentQuery {
	query := (&ElectionClient{config: cq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if cq.withParent != nil || cq.withUser != nil || cq.withElection != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := cq.withElection; query != nil {
		if err := cq.loadElection(ctx, query, nodes, nil,
			func(n *Comment, e *Election) { n.Edges.Election = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadElection(ctx context.Context, query *ElectionQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Election)) error {
	ids := make([]int, 0, len(nodes))
	nodeids := make(map[int][]*Comment)
	for i := range nodes {
		if nodes[i].election_comments == nil {
			continue
		}
		fk := *nodes[i].election_comments
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "election_comments" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
