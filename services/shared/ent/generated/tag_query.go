...
// QueryElections chains the current query on the "elections" edge.
func (tq *TagQuery) QueryElections() *ElectionQuery {
	query := (&ElectionClient{config: tq.config}).Query()
...
func (tq *TagQuery) Clone() *TagQuery {
	...
	return &TagQuery{
		...
		withElections: tq.withElections.Clone(),
...
// WithElections tells the query-builder to eager-load the nodes that are connected to
// the "elections" edge. The optional arguments are used to configure the query builder of the edge.
func (tq *TagQuery) WithElections(opts ...func(*ElectionQuery)) *TagQuery {
	query := (&ElectionClient{config: tq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	tq.withElections = query
	return tq
}
...
func (tq *TagQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Tag, error) {
	...
	if query := tq.withElections; query != nil {
		if err := tq.loadElections(ctx, query, nodes,
			func(n *Tag) { n.Edges.Elections = []*Election{} },
			func(n *Tag, e *Election) { n.Edges.Elections = append(n.Edges.Elections, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (tq *TagQuery) loadElections(ctx context.Context, query *ElectionQuery, nodes []*Tag, init func(*Tag), assign func(*Tag, *Election)) error {
	...
	query.Where(func(s *sql.Selector) {
		joinT := sql.Table(tag.ElectionsTable)
		s.Join(joinT).On(s.C(election.FieldID), joinT.C(tag.ElectionsPrimaryKey[1]))
		s.Where(sql.InValues(joinT.C(tag.ElectionsPrimaryKey[0]), edgeIDs...))
		columns := s.SelectedColumns()
		s.Select(joinT.C(tag.ElectionsPrimaryKey[0]))
	...
	neighbors, err := withInterceptors[[]*Election](ctx, query, qr, query.inters)
	if err != nil {
		return err
	}
	for _, n := range neighbors {
		nodes, ok := nids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected "elections" node returned %v`, n.ID)
		}
		for kn := range nodes {
			assign(kn, n)
		}
